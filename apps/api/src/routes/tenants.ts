...
export default async function tenantRoutes(app: FastifyInstance) {
  ...
  app.get('/', { preHandler: [requireRole('superadmin', 'support', 'sales')] }, tenantController.list)

  // GET /api/v1/tenants/:id
  app.get('/:id', { preHandler: [requireRole('superadmin', 'support')] }, tenantController.getById)
  ...
  app.post('/:id/impersonate', { preHandler: [requireRole('superadmin', 'support')] }, tenantController.impersonate)
}
